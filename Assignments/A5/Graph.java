...
public class Graph implements GraphADT {
	...
	private GraphEdge adjMat[][];	//Adjacency Matrix object for storing information about which vertices are connected to which, as well as the relationships between them
	private GraphNode nodeArr[];		//Array to store the nodes of the graph
	...
	public Graph(int n) {
		...
		adjMat = new GraphEdge[n][n];		//Initialize the adjacency matrix with the passed number
		nodeArr = new GraphNode[n];			//Initialize the node array to the passed size
		
		for(int i = 0; i < n; i++){		//From 0 to the number of nodes
			nodeArr[i] = new GraphNode(i);	//Add a node to the current index of the array and initialize it to the current index #
		}
	...
	public void insertEdge(GraphNode u, GraphNode v, char busLine) throws GraphException {
		if(inGraph(u, v)){		//If the passed nodes exist within the graph, do the following
			GraphEdge e = adjMat[u.getName()][v.getName()];	//Get the element stored between these two nodes (will return an edge or null)
			if(e == null){														//If the element was null, we can add the edge
				adjMat[u.getName()][v.getName()] = new GraphEdge(u, v, busLine);		//Create an edge between the first and second with the passed data
				adjMat[v.getName()][u.getName()] = new GraphEdge(v, u, busLine);		//Create an edge between the second and first with the passed data (accounts for both directions)
			}
	...
	public GraphNode getNode(int n) throws GraphException {	
	...
	public GraphEdge getEdge(GraphNode u, GraphNode v) throws GraphException {
			if(inGraph(u, v)){	//If both nodes are in the graph
				GraphEdge result = adjMat[u.getName()][v.getName()];	//get the edge between these two nodes
	...
	public boolean areAdjacent(GraphNode u, GraphNode v) throws GraphException {
		...
		if(inGraph(u, v)){	//If both nodes are in the graph
			GraphEdge result = adjMat[u.getName()][v.getName()];	//Get the edge between the nodes
	...
	private boolean inGraph(GraphNode u, GraphNode v) {
	...
	private boolean inGraph(GraphNode u) {
		try{
			getNode(u.getName());
			return true;
		}
